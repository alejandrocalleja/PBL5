...
public class UserRepositoryImpl implements UserRepositoryCustom {
   ...
   * createNativeQuery("SELECT * FROM user WHERE username=? AND password=?",
   * UserModel.class);
   * 
   ...
   * 
   * Query query = entityManager.
   * createQuery("SELECT COUNT(*) FROM FollowRelation WHERE followed_id=?1");
   * query.setParameter(1, user_id);
   * 
   * try {
   ...
   * 
   * Query query = entityManager.
   * createQuery("SELECT COUNT(*) FROM FollowRelation WHERE follower_id=?1");
   * query.setParameter(1, user_id);
   * 
   * try {
   * result = ((Number) query.getSingleResult()).intValue();
